...
export function runTests() {
  ...
  test('All spawns, resources, and NPC doors are walkable',()=>{
    const w=createWorld();for(const p of [w.player,...w.enemies,...w.npcs,...w.map.resources,...w.map.buildings.map(b=>b.door)])assert(walkable(w,p.x,p.y),p.id||JSON.stringify(p));
  });
  test('Continuous movement routes around solid buildings',()=>{
    const w=createWorld(),p=w.player;p.x=28;p.y=30;const goal={x:34,y:30};
  ...
  test('Cleave hits nearby enemies; Backstab requires sneak and rear position',()=>{
    const w=createWorld(),p=w.player,e=w.enemies[0],other=w.enemies[1];e.x=35;e.y=25;p.x=e.x;p.y=e.y-1;other.x=e.x+0.7;other.y=e.y;w.enemies=[e,other];p.points=2;unlock(p,'cleave');unlock(p,'backstab');p.targetId=e.id;
  ...
  test('Noncombat gathering, sale, crafting and equipment work',()=>{
    const w=createWorld(),p=w.player,node=w.map.resources[0];p.x=node.x;p.y=node.y;assert(gather(w,node,notify),'Gather failed');assert(!gather(w,node,notify),'Gather farming allowed');
  ...
  test('Death retains progression and applies temporary penalty',()=>{
    const w=createWorld();w.player.level=4;w.player.health=0;respawnPlayer(w,notify);assert(w.player.level===4&&w.player.penalty===45,'Death lost progression');assert(distance(w.player,w.map.spawn)===0&&w.player.health>0,'No respawn');
  });
  test('NPCs reach work and home on daily schedule',()=>{
    const w=createWorld();for(let i=0;i<200;i++)updateNPCs(w,0.25);
  ...
  test('Save round-trip preserves progression, NPCs, loot, cooldowns and depletion',()=>{
    const w=createWorld();let raw=null;const storage={setItem(k,v){raw=v;},getItem(){return raw;}};
