...
function interact() {
  ...
  } else if(pending.type==='npc' && (!world.player.destination||distance(world.player.destination,entity)>1))setDestination(world,world.player,entity);
  else if(!world.player.path.length){world.interaction=null;ui.notify('No clear route to that object.');}
}
installInput(canvas,camera,()=>world,()=>world&&!menuOpen&&!ui.panel,ui.notify);
